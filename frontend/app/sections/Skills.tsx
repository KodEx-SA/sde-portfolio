      ...
      <div className="max-w-6xl mx-auto relative z-10">
        ...
        <div className="flex flex-col gap-3 mb-14">
          ...
          <h2 className="text-4xl md:text-5xl font-black text-white font-mono">
            Skills &amp; <span className="text-brand-400">Tools</span>
          </h2>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {skillGroups.map((group) => (
            <div
              key={group.category}
              className="relative p-5 rounded-xl border border-brand-500/10 bg-[#080808] hover:border-brand-500/25 transition-all duration-300 group overflow-hidden"
            >
              <div className="absolute inset-0 bg-brand-500/2 opacity-0 group-hover:opacity-100 transition-opacity duration-300 rounded-xl" />
              <div className="absolute top-2 left-2 w-3 h-3 border-t border-l border-brand-500/25 group-hover:border-brand-500/50 transition-colors duration-300" />
              <div className="absolute bottom-2 right-2 w-3 h-3 border-b border-r border-brand-500/25 group-hover:border-brand-500/50 transition-colors duration-300" />

              <div className="relative z-10">
                <p className="text-brand-700 font-mono text-[9px] mb-1">{group.command}</p>
                <h3 className="text-white font-mono font-bold text-sm mb-4">{group.category}</h3>
                <div className="flex flex-wrap gap-1.5">
                  {group.skills.map((skill) => (
                    <span
                      key={skill}
                      className="px-2.5 py-1 text-[10px] font-mono bg-brand-500/6 text-brand-400 rounded-md border border-brand-500/12 hover:border-brand-500/30 hover:bg-brand-500/12 transition-all duration-200 cursor-default"
                    >
                      {skill}
                    </span>
                  ))}
        ...
        <div className="mt-16 grid grid-cols-2 sm:grid-cols-4 gap-6">
          {stats.map((stat) => (
            <div key={stat.label} className="text-center group">
              <p className="text-4xl font-black font-mono text-brand-400 group-hover:text-brand-300 transition-colors duration-200">{stat.value}</p>
